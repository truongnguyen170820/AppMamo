...
class WithdrawalHistoryBloc implements Bloc {
  Subject _subject = BehaviorSubject<ApiResponse<JDIResponse>>();
  StreamController _getWithdrawalHistoryStreamCtrl =
      StreamController<List<TransactionHistoryModel>>();
  Stream<List<TransactionHistoryModel>> get getWithdrawalHistoryStream =>
      _getWithdrawalHistoryStreamCtrl.stream;

  getWithdrawalHistory(int pageIndex, int pageSize) {
    ApiService(ApiConstants.TRANSACTION_HIS,
        {"Type": 3, "PageIndex": pageIndex, "PageSize": pageSize}, _subject)
        .execute();
  }
          ...
          if (jdiResponse.ErrorCode == AppConstants.ERROR_CODE_SUCCESS) {
            List<TransactionHistoryModel> result =
                jdiResponse.Data.map((e) => TransactionHistoryModel.fromJson(e))
                    .toList();
            _getWithdrawalHistoryStreamCtrl.sink.add(result);
          } else {
            Utilities.showToast(
  ...
  @override
  void dispose() {
    _getWithdrawalHistoryStreamCtrl.close();
  }
}
